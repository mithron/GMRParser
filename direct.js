...
var write = function(args) {
 var filename = (typeof args.name === "undefined") ? "debug" : args.name;
 var accessmode = (typeof args.mode === "undefined") ? "w" : args.mode;
 var data = (typeof args.data === "undefined") ? "w" : args.data;
 
 fs.write( filename + debugcounter.getTime() + '.html',  data, {
  opts: {
    mode: accessmode, 
    charset: 'UTF-8' 
  }
...
if (fs.exists(addr_path)) {
	...
	while(!addr_file.atEnd() && counter <= 10) {
		...
		casper.waitForSelector('table.tab3 > tbody > tr:nth-child(4)', function get_house_data() {		
				write({'data':  this.getHTML('table.tab3 > tbody > tr:nth-child(4)'), 'name': 'floor' });
				// ”правление
		...
		casper.waitForSelector('div.p3 > div > a.h4', function get_governor_data() {
				write( {'data': this.getHTML('div.p3 > div > a.h4'), 'name': 'governor'} );
			}, skip);
